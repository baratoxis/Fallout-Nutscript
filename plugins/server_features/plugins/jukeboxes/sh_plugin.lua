local PLUGIN = PLUGIN
PLUGIN.name = "Jukeboxes"
PLUGIN.desc = "Add jukeboxes"
...
nut.config.add("radioUrl", "http://fallout.fm:8000/falloutfm6.ogg", "The radio url used by radios and jukeboxes.", function()
    if ( CLIENT ) then
        for _, v in pairs(PLUGIN.activeRadios) do
            v:StopStream()
            v:StartStream()
